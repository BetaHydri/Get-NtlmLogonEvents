...
All settings are located under: `Computer Configuration\Policies\Windows Settings\Security Settings\Local Policies\Security Options`

> **Path abbreviation:** `CCS` = `CurrentControlSet`

### Auditing Policies (enable first — safe, audit-only)

| GPO Setting | Registry Path | Values | Recommended | Scope |
|---|---|---|---|---|
| Restrict NTLM: Audit Incoming NTLM Traffic | `HKLM\SYSTEM\CCS\Control\Lsa\MSV1_0\AuditReceivingNTLMTraffic` | 0=Disable, 1=Domain accounts, 2=All accounts | **1** — Enable auditing for domain accounts (safe to enable) | All devices |
| Restrict NTLM: Outgoing NTLM traffic to remote servers | `HKLM\SYSTEM\CCS\Control\Lsa\MSV1_0\RestrictSendingNTLMTraffic` | 0=Allow all, 1=Audit all, 2=Deny all | **1** — Audit all (safe — audit only) | All devices |
| Restrict NTLM: Audit NTLM authentication in this domain | `HKLM\SYSTEM\CCS\Services\Netlogon\Parameters\AuditNTLMInDomain` | 0=Disable, 1=Domain→domain servers, 3=Domain accounts, 5=Domain servers, 7=All | **7** — Enable all (safe to enable) | Domain Controllers only |

### Blocking Policies (enforce after remediation — test thoroughly first)

| GPO Setting | Registry Path | Values | Recommended | Scope |
|---|---|---|---|---|
| Restrict NTLM: Incoming NTLM traffic | `HKLM\SYSTEM\CCS\Control\Lsa\MSV1_0\RestrictReceivingNTLMTraffic` | 0=Allow all, 1=Deny all domain accounts, 2=Deny all accounts | **1** — Deny all domain accounts (final goal — configure only after auditing is complete) | All devices (with exceptions if needed) |
| Restrict NTLM: NTLM authentication in this domain | `HKLM\SYSTEM\CCS\Services\Netlogon\Parameters\RestrictNTLMInDomain` | 0=Disable, 1=Domain→domain servers, 3=Domain accounts, 5=Domain servers, 7=All | **7** — Deny all (final goal — configure only after auditing is complete) | Domain Controllers only |

### Exception Lists

| GPO Setting | Registry Path | Scope | Notes |
|---|---|---|---|
| Add remote server exceptions for NTLM authentication | `HKLM\...\MSV1_0\ClientAllowedNTLMServers` | All devices | Servers allowed for outbound NTLM when blocking is enabled. Supports wildcards and SPN format (e.g., `cifs/server.contoso.com`). Minimize exceptions. |
| Add server exceptions in this domain | `HKLM\...\Netlogon\Parameters\DCAllowedNTLMServers` | Domain Controllers only | Servers exempted from domain-wide NTLM restrictions. Minimize exceptions. |

### NTLM Operational Events Generated by Each Policy

| Auditing Policy | Events Generated | Description |
|---|---|---|
| Audit Incoming NTLM Traffic | 8002 (local account), 8003 (domain account) | Logs incoming NTLM authentication attempts on servers |
| Outgoing NTLM traffic = Audit all | 8001 | Logs outgoing NTLM authentication attempts from clients |
| Audit NTLM authentication in this domain = Enable all | 8004 (credential validation), 8005 (direct DC auth), 8006 (cross-domain) | Logs NTLM activity at the domain controller level |
| Incoming NTLM traffic = Deny | 4002 (local), 4003 (domain) | Logs **blocked** incoming NTLM attempts |
| Outgoing NTLM traffic = Deny all | 4001 | Logs **blocked** outgoing NTLM attempts |
| NTLM authentication in this domain = Deny | 4004 (validation), 4005 (direct), 4006 (cross-domain) | Logs **blocked** NTLM at DC level |
